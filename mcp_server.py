...
class FileModificationRequest(BaseModel):
    ...
    description: str
    chat_id: str
    repo_name: str = None  # Opcional, usa o repositório atualmente selecionado se não especificado
...
def get_repo_for_user(chat_id: str, repo_name: str = None):
    ...
    if chat_id not in user_current_repos:
        return None, "Nenhum repositório selecionado. Use /repos para listar e /select para escolher um."
    ...
    if repo_name:
        ...
        if not os.path.isdir(repo_path) or not os.path.isdir(os.path.join(repo_path, '.git')):
            return None, f"Repositório '{repo_name}' não encontrado ou não é um repositório Git válido."
...
def run_github_action(workflow_name: str):
    ...
    payload = {
        "ref": "main"  # Ou a branch que você está usando
    }
...
async def select_repo(request: RepoSelectionRequest):
    ...
    try:
        ...
        if not os.path.isdir(os.path.join(repo_path, '.git')):
            error_msg = f"O diretório '{request.repo_name}' não é um repositório Git válido."
            send_telegram_message(request.chat_id, error_msg)
...
@app.post("/suggest")
async def suggest_modification(request: FileModificationRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return {"status": "processing", "message": "Processando solicitação de sugestão"}
...
async def process_suggestion_request(file_path: str, description: str, chat_id: str, repo_path: str):
    ...
    try:
        ...
        if "```" in suggested_code:
            ...
            if len(parts) >= 3:
                ...
                if suggested_code.startswith(("python", "javascript", "java", "typescript")):
...
async def apply_modification(request: ApplyModificationRequest):
    ...
    try:
        ...
        return {"status": "success", "message": f"Sugestão #{request.suggestion_id} aplicada com sucesso"}
...
async def reject_modification(request: ApplyModificationRequest):
    ...
    try:
        ...
        return {"status": "success", "message": f"Sugestão #{request.suggestion_id} rejeitada"}
...
async def commit_changes(request: CommitRequest):
    ...
    try:
        ...
        return {"status": "success", "message": f"Commit realizado com sucesso: '{request.message}'"}
...
async def push_changes(request: PushRequest):
    ...
    try:
        ...
        return {"status": "success", "message": "Alterações enviadas com sucesso para o GitHub"}
...
async def execute_github_action(workflow_name: str, request: PushRequest):
    ...
    try:
        ...
        if result:
            ...
            return {"status": "success", "message": f"GitHub Action '{workflow_name}' iniciada com sucesso"}
        else:
