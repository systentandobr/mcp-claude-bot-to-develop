...
async def get_api_key(api_key_header: str = Security(api_key_header)):
    ...
    if api_key_header != MCP_API_KEY:
        raise HTTPException(
            status_code=403, detail="API Key inválida ou não autorizada."
        )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if not all([api_key, timestamp, signature]):
                raise HTTPException(
                    status_code=401, detail="Cabeçalhos de autenticação incompletos."
                )
            ...
            if abs(current_time - request_time) > MAX_TIMESTAMP_DIFF:
                raise HTTPException(
                    status_code=401, detail="Timestamp expirado ou inválido."
                )
            ...
            expected_signature = hmac.new(
                MCP_API_KEY.encode(), message.encode(), hashlib.sha256
            ).hexdigest()
